...
export default function ChatBot({ token }: { token: string | null }) {
        ...
        <CardContent className="p-0">
              ...
              <div key={idx} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[80%] p-3 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${
                    msg.role === "user" ? "bg-[hsl(var(--periwinkle))] text-white" : "bg-[hsl(var(--lavender))] text-black"
                  }`}
                >
          ...
          <div className="p-4 border-t-4 border-black bg-white">
            <div className="flex gap-2">
              ...
              />
              <Button onClick={handleSend} disabled={isLoading || !input.trim() || !token} size="icon" className="bg-[hsl(var(--indigo))] hover:bg-[hsl(var(--violet))] text-white border-4 border-black">
